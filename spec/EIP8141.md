...
> `064f49621d05ce25323def867a6a2ed9275d3570` with one local normative change: the native
> `SIGDATACOPY` split at `0xb5`. It otherwise retains the pinned scalar transaction payload,
> frame modes, and selectors. The final appendix separately records a **non-normative,
> context-only tooling fixture** inspired by EIP-8250/8272/7906; it does not extend the wire
> transaction specified below. `tools/check-spec-drift.sh` compares current upstream
> EIP-8141 with the exact upstream pin, not with this document. See
> [`VERSIONS.md`](../VERSIONS.md) for divergences and reproducibility status.
...
The raw `signature` byte strings of protocol-validated schemes are intentionally not introspectable by EVM code to allow future aggregation schemes. Contracts can inspect only the metadata of protocol-validated signature entries through transaction introspection. The raw `signature` byte strings of `ARBITRARY` entries are introspectable by EVM code through `SIGDATACOPY`. This means that they cannot be aggregated in the future.
...
This instruction gives access to signature-scoped metadata. The raw `signature` bytes of protocol-validated schemes are intentionally not accessible from the EVM. `ARBITRARY` signature bytes are validated during EVM execution and can be accessed through `SIGDATACOPY`.

The gas cost of this operation is `2`. It takes two values from the stack, `signatureIndex` on top and `param` second from top, and returns the associated value to the stack.
...
This opcode copies an `ARBITRARY` signature entry's raw `signature` bytes into memory. Its gas cost is calculated exactly as for `CALLDATACOPY`, including the fixed cost of 3, the per-word copy cost, and standard EVM memory expansion.
...
If `signatureIndex` is out of bounds or the referenced entry's `scheme` is not `ARBITRARY`, an exceptional halt occurs. The operation semantics match `CALLDATACOPY`, copying `length` bytes from the chosen signature's raw bytes, starting at `dataOffset`, into memory starting at `memOffset`. Bytes beyond the end of the signature are copied as zeroes.
...
| sender legacy nonce | `TXPARAM(0x0C)` | Sender account nonce in transaction pre-state |
| nonce-key list | `TXPARAM(0x0D)`, `TXPARAM(0x10)` | Supplied `uint256` keys; selectors return the count and first key |
| nonce-key hash | `TXPARAM(0x0E)` | Supplied `bytes32` value intended to represent the key list |
| recent-root references | `TXPARAM(0x0F)`, `RECENTROOTREFLOAD` | Supplied `(source_id, slot, root)` records |
...
Fixture-only `TXPARAM` selectors are:

| `param` | Return value |
|---|---|
| `0x0C` | sender legacy nonce |
| `0x0D` | number of supplied nonce keys |
| `0x0E` | supplied nonce-key hash |
| `0x0F` | number of supplied recent-root references |
| `0x10` | first supplied nonce key; exceptional halt when the list is empty |

Other selectors above `0x10` are undefined. All baseline selectors keep the semantics in the
normative body except that fixture `TXPARAM(0x01)` reports the supplied shared sequence.
