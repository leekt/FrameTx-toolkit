...
The toolkit follows this table exactly: `0x03` through `0xff` remain reserved and are
rejected rather than interpreted as local signature schemes.
...
#### Post-quantum signatures in this toolkit (non-normative note)

ML-DSA is not a protocol-validated signature scheme in EIP-8141 or this toolkit. An
application experimenting with ML-DSA must use an `ARBITRARY` entry with an empty `signer`,
copy its application-defined witness with `SIGDATACOPY`, and verify it in a `VERIFY` frame
or a custom verifier contract. That validation must bind the proof to the canonical
transaction signature hash when it authorizes the transaction.

No canonical ML-DSA witness encoding, verifier, account, paymaster, signer identity, or gas
schedule is defined or shipped here. A future pure-function validation design could provide
a structured, cacheable verifier path without consuming a reserved signature-scheme value;
it is outside the current EIP-8141 specification.
...
The default code is deliberately **secp256k1-only**. A post-quantum account or payer must
...
the same empty-`msg` secp256k1 authorization. An `ARBITRARY` post-quantum witness cannot use
the empty-code payer path, because default code still requires `SECP256K1` at index 1.
...
The fixture does not add `TXPARAM` selectors. It implements only the normative selector
table (`0x00` through `0x0C`); every other selector is undefined and exceptional-halts.
