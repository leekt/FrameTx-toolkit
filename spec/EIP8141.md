...
> **Toolkit document status.** The normative body is upstream EIP-8141 at
> `f767a1e8078e17c9b381a91d35a09492189ede1b` (official `ethereum/EIPs` master on
> 2026-08-23). It includes the merged native `SIGDATACOPY` instruction at `0xb5`, the
> current frame-entry access and precompile-dispatch rules, uniform calldata-floor token
> accounting, and the current validation-prefix restrictions.
> It carries the master-spec envelope: `fees` and `limits` sublists, EIP-8037 state-gas
> budgets, two-dimensional receipts, and the EIP-7825 execution cap. The final appendix
> separately records a **non-normative, context-only tooling fixture** inspired by
> EIP-8250/8272/7906; it does not extend the wire transaction specified below.
> A separate section below also defines the toolkit's **non-normative, experimental
> ML-DSA-44 signature scheme at reserved value `0x03`**. Upstream EIP-8141 still reserves
> that value; transactions using it are portable only across clients implementing this
> exact local profile.
> `tools/check-spec-drift.sh` compares current upstream EIP-8141 with the exact upstream
> pin, not with this document. See [`VERSIONS.md`](../VERSIONS.md) for divergences and
...
The reservation in this normative table is unchanged. The toolkit-local
[ML-DSA-44 profile](#toolkit-local-ml-dsa-44-scheme-0x03-non-normative) below assigns
`0x03` only inside this implementation.
...
#### Toolkit-local ML-DSA-44 scheme `0x03` (non-normative)

> **Experimental local extension.** Upstream EIP-8141 reserves scheme `0x03`. This
> subsection is not a proposed allocation and is not consensus-compatible with an upstream
> client. It records the exact behavior of this toolkit so its test vectors and raw
> transactions are reproducible.

The toolkit interprets scheme `0x03` as pure-mode ML-DSA-44 from
[NIST FIPS 204](https://csrc.nist.gov/pubs/fips/204/final). Its raw `signature` field is one
fixed byte string:
...
where `sigma` is the 2,420-byte FIPS 204 ML-DSA-44 signature encoding and `public_key` is
the 1,312-byte FIPS 204 ML-DSA-44 public-key encoding. The field is therefore exactly
3,732 bytes. There is no ABI wrapper, length prefix, DER form, public-key alias, or trailing
data.

The entry follows the same outer metadata rules as the other protocol-validated schemes:
`signer` is either empty or exactly 20 bytes, and `msg` is either empty or a non-zero
32-byte explicit digest. An empty signer resolves to `tx.sender`; an explicit signer
resolves to itself. The ML-DSA public key's identity is:

```text
key_id = low20(keccak256(0x03 || public_key))
```

The one-byte `0x03` domain prefix is part of the hash. Validation requires
`resolved_signer == key_id`.

The verifier uses FIPS 204 **pure ML-DSA**, not HashML-DSA. It supplies an empty context and
treats the already-selected 32-byte FrameTx message as the FIPS message byte string without
another prehash:
...
ML_DSA_44_WIRE_SIZE = 3732

def validate_toolkit_ml_dsa_44(sig, tx_sender, sig_hash) -> bool:
    if sig.scheme != ML_DSA_44 or len(sig.signature) != ML_DSA_44_WIRE_SIZE:
        return False

    if len(sig.msg) == 0:
        message = sig_hash
    elif len(sig.msg) == 32 and sig.msg != b"\x00" * 32:
        message = sig.msg
    else:
        return False
    ...
    sigma = sig.signature[:ML_DSA_44_SIGNATURE_SIZE]
    public_key = sig.signature[ML_DSA_44_SIGNATURE_SIZE:]
    if resolved_signer != keccak256(b"\x03" + public_key)[12:]:
        return False

    # Apply the exact FIPS 204 ML-DSA-44 decoders. Reject a decoding failure or
    # any byte string that is not its unique canonical encoding, including a
    # signature whose hint indices are repeated or not strictly increasing.
    pk = ML_DSA_44_PK_DECODE(public_key)
    decoded_signature = ML_DSA_44_SIG_DECODE(sigma)
    if pk is BOTTOM or decoded_signature is BOTTOM:
        return False
    if ML_DSA_44_PK_ENCODE(pk) != public_key:
        return False
    if ML_DSA_44_SIG_ENCODE(decoded_signature) != sigma:
        return False
...
Toolkit structural validation admits `0x03` alongside `SECP256K1` and `P256`, and toolkit
signature validation dispatches to the function above. Its provisional signature-verification
charge is **50,000 gas**, so the toolkit's `signature_gas(sig)` returns `50000` for scheme
`0x03`; normal signature-byte calldata and calldata-floor charges still apply. This number is
a local benchmark placeholder, not an upstream gas proposal.

These are separate constraints. The 3,732-byte `sigma || public_key` field contributes to
intrinsic data gas and the calldata floor, while 50,000 counts toward the current
`MAX_VERIFY_GAS = 100000` validation-prefix budget. One ML-DSA-44 entry plus one secp256k1
entry consumes 52,800 of that budget before frame code, leaving at most 47,200 for a hybrid
multisig's validation execution. Two ML-DSA-44 entries consume all 100,000 before any VERIFY
frame executes and therefore do not satisfy the current public-mempool limit.

The current verifier pins RustCrypto [`ml-dsa` version
`0.1.1`](https://github.com/RustCrypto/signatures/tree/master/ml-dsa). RustCrypto explicitly
states that this implementation has never been independently audited. Version `0.1.1` is
newer than the fix for
[GHSA-5x2r-hc65-25f9](https://github.com/RustCrypto/signatures/security/advisories/GHSA-5x2r-hc65-25f9),
which affected versions through `0.1.0-rc.3` by accepting repeated hint indices, but pinning
a patched dependency does not make this toolkit implementation audited. Scheme `0x03` and
the contracts built on it are experimental and must not protect production funds.
...
The default code is deliberately **secp256k1-only**. The toolkit-local ML-DSA-44 scheme
`0x03` does not widen it: a 20-byte ML-DSA key identity may be a frame target, but an
empty-code target still rejects that signature scheme. A post-quantum account or payer must
therefore have validating code (or a delegation to such code) unless a future specification
changes the default-code rule.

One envelope entry may nevertheless serve more than one validation policy. In particular,
a `SECP256K1` entry at signature index 1 can both count as an owner signature in an earlier
`MultisigAccount.validate(uint256[])` execution-approval frame and authorize that same
owner's later empty-code PAYMENT frame. For this layout:

- index 1 has an empty `msg`, an explicit 20-byte `signer` equal to the codeless payer/owner,
  and one canonical `v || r || s` signature over `compute_sig_hash(tx)`;
- the multisig's signed index array includes `1`, in whatever signer-address order its
  threshold policy requires; and
- the later PAYMENT-only frame targets that signer address, causing default code to select
  index 1.

Both frames refer to the same signature entry and the same canonical signature bytes; the
signer does not provide a second signature. This reuse works because both policies demand
the same empty-`msg` secp256k1 authorization. It does not let an ML-DSA-44 owner use the
empty-code payer path, because default code still requires `SECP256K1` at index 1.
...
and revm host interface. It is not part of the normative EIP-8141 overlay above, does not
...
The baseline Anvil path can project its scalar nonce into a compatibility fixture containing
the single nonce key `0`, while leaving recent-root references and the trace empty. That
projection is tooling scaffolding, not keyed-nonce transaction support.
...
|---|---|---|
| shared nonce sequence | `TXPARAM(0x01)` | Host-supplied sequence shared by the supplied nonce keys; replaces the scalar interpretation only inside this fixture profile |
| sender legacy nonce | `TXPARAM(0x80)` | Sender account nonce in transaction pre-state |
| nonce-key list | `TXPARAM(0x81)`, `TXPARAM(0x84)` | Supplied `uint256` keys; selectors return the count and first key |
| nonce-key hash | `TXPARAM(0x82)` | Supplied `bytes32` value intended to represent the key list |
| recent-root references | `TXPARAM(0x83)`, `RECENTROOTREFLOAD` | Supplied `(source_id, slot, root)` records |
...
nonce-key hash, enforce nonce-key ordering, verify recent roots, or construct a trace from
execution. Callers are responsible for supplying internally consistent fixture data.
...
Fixture-only `TXPARAM` selectors sit at `0x80` and above, clear of the normative table so
newly assigned normative selectors never collide with them:

| `param` | Return value |
|---|---|
| `0x80` | sender legacy nonce |
| `0x81` | number of supplied nonce keys |
| `0x82` | supplied nonce-key hash |
| `0x83` | number of supplied recent-root references |
| `0x84` | first supplied nonce key; exceptional halt when the list is empty |

Selectors between the normative table and `0x80`, and above `0x84`, are undefined. All
baseline selectors keep the semantics in the normative body except that fixture
`TXPARAM(0x01)` reports the supplied shared sequence.
