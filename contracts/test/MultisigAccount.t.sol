...
/// examples/04-multisig-account, executed against the real opcodes.
///
/// The account runs no `ecrecover`. Every entry in `tx.signatures` was already
/// verified by the protocol against the canonical signature hash, so these tests
/// hand it verified entries and assert only on the policy: count distinct owner
/// signatures over THIS transaction, and approve once `threshold` is reached.
